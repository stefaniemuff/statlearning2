...
For the least square estimator, the solution can be found in the first section [here](https://en.wikipedia.org/wiki/Proofs_involving_ordinary_least_squares).
...
```{r, message=FALSE, warning=FALSE}
# Exclude 'ID' column
credit_data <- subset(Credit, select = -ID)
...
set.seed(1)
train_perc <- 0.75
credit_data_train_index <- sample(
  nrow(credit_data),
  round(nrow(credit_data) * train_perc)
)
# Create train and test set
credit_data_training <- credit_data[credit_data_train_index, ]
credit_data_testing <- credit_data[-credit_data_train_index, ]
```
...
best_subset_method <- regsubsets(Balance ~ .,
  ...
  nvmax = credit_data_number_predictors
)
...
k <- 10
set.seed(1)
folds <- sample(k, nrow(credit_data_training), replace = TRUE)
cv.errors <-
  matrix(NA,
    nrow = k,
    ncol = credit_data_number_predictors,
...
for (j in seq_len(k)) {
  best_subset_method <- regsubsets(Balance ~ .,
    data = credit_data_training[folds != j, ],
    nvmax = credit_data_number_predictors
  )
  for (i in seq_len(credit_data_number_predictors)) {
    pred <- predict(best_subset_method,
      credit_data_training[folds == j, ],
      id = i
    )
