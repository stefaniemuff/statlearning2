...
2. For $b=1,2,...,B$, repeat:  
    ...
    b) Update $\hat{f}$ by adding in a shrunken version of the new tree: $$\hat{f}(x) \leftarrow \hat{f}(x)+\eta \hat{f}^b(x).$$  
    c) Update the residuals, $$r_i \leftarrow r_i - \eta \hat{f}^b(x_i).$$  
3. The boosted model is $\hat{f}(x) = \sum_{b=1}^B \eta \hat{f}^b(x).$

Boosting has three tuning parameters which need to be set ($B$, $\eta$, $d$), and can be found using cross-validation.
...
* **Shrinkage parameter $\eta$**. Controls the rate at which boosting learns. $\eta$ scales the new information from the $b$-th tree, when added to the existing tree $\hat{f}$. Choosing a small value for $\eta$ ensures that the algorithm learns slowly, but will require a larger $B$. Typical values of $\eta$ is 0.1 or 0.01.
...
* Calculate the MSE on the test set, first for the model with $\eta=0.001$ (default), then with $\eta=0.2$.

* Ideally, we should do a cross-validation to find the best $\eta$ over a grid (comes later), but here it seems not to make a big difference.
...
c) Number of trees, i.e., the number of boosting iterations
...
* Moreover, by giving tree depth and tuning the number of trees, we can hope to get this parameter "for free".
...
## c) The number of tress (regularization strategy 1)
...
* $\eta$ and $M$ are connected: Smaller $\eta$ requires larger $M$, and vice versa. 

\vspace{2mm}

* Empirically, it was found that small $\eta$ (i.e., $\eta<0.1$) is more robust. 

\vspace{2mm}

* Possible strategy: Fix $\eta$ and then choose $M$ by early stopping. 
...
* Stochastic gradient boosting: chose a random subsample of the training data for building each tree [@friedman2002].
...
The proportion $\eta$ is _another hyper-parameter_ that can be tuned...
...
* _\textcolor{red}{Regularization}_: XGBoost includes different regularization penalties to avoid overfitting. Each new tree is regularized with two terms: The number of leafs (pruning) and the weights on the leafs. The total penalizing term is typically given by
...
* Pruning parameter $\gamma$: the new tree is grown to the specified depth, and then pruned to find and remove splits according to the contraint imposes by $\gamma$.
...
* Tuning the models become very slow! We refer to @boehmke_greenwell2020 for parameter tuning runs:   
https://bradleyboehmke.github.io/HOML/gbm.html
