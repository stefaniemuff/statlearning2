...
\vspace{2mm}
Study with 243 male participants, where body fat (%) and BMI and other predictors were measured. Some scatterplots\footnote{The data to reproduce these plots and analyses can be found here: https://github.com/stefaniemuff/statlearning2/tree/master/3LinReg/data}:
...
In this formulation $Y$ is a random variable $Y | x\sim N(\beta_0 + \beta_1 x, \sigma^2$) where
$$Y|x \quad= \quad \underbrace{\text{ expected value }}_{\mathsf{E}(Y) = \beta_0 + \beta_1 x} \quad + \quad \underbrace{\text{ error}}_{\varepsilon}  \ .$$
...
and 
$$
 \hat{\beta}_1 = \frac{\sum_{i=1}^n(x_i-\bar{x})(y_i-\bar{y})}{\sum_{i=1}^n(x_i-\bar{x})^2} = \frac{\mathsf{Cov}(\bf{x},\bf{y})}{\mathsf{Var}(\bf{x})}\ , 
$$
...
2. The **coefficient of determination $R^2$**, which measures the proportion of $y$'s variance explained by the model (between 0 and 1), is a **relative measure** of _fit_ or _lack of fit_ ($1-R^2$):
...
* What is the expected value $\mathsf{E}(\bf Y)$ given $\bf{X}$?

* The covariance matrix $\mathsf{Cov}(\bf Y)$ given $\bf{X}$?

* Thus what is the distribution of $\bf Y$ given $\bf{X}$?
...
$\rightarrow$ Derivation on next 2 pages. 
...
```{r,echo=FALSE, fig.width=8, fig.height=3.5,fig.align = "center",out.width='90%'}
library(ggfortify)
autoplot(r.bodyfat,smooth.colour=NA)[1] + theme_bw() 
```
...
```{r,echo=FALSE, fig.width=8, fig.height=3.5,fig.align = "center",out.width='90%'}
autoplot(r.bodyfat,smooth.colour=NA)[2] + theme_bw() 
```
...
```{r,echo=FALSE, fig.width=8, fig.height=3.5,fig.align = "center",out.width='90%'}
autoplot(r.bodyfat,which=5) + theme_bw()
```
...
* Exercise: Verify that formula (2) comes out in the special case of simple linear regression.
