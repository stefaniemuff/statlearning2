...
Now, let us look at the `Wage` data set. The section on Additive Models [(in this week's slides)](https://github.com/stefaniemuff/statlearning2/blob/master/7BeyondLinear/7BeyondLinear_2024.pdf) explains how we can create an additive model by adding components together. One component we saw is a natural spline in `year` with one knot. Derive the expression for the design matrix $\mathbf X_2$ from the natural spline basis:
$$
