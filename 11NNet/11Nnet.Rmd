...
where the gradient is calculated via _\textcolor{red}{backpropagation}_. 
...
* Small batches also bring in a _\textcolor{red}{regularization effect}_, due to the variability they bring to the optimization process.
...
<!--     c) Compute the gradient $\nabla_{\boldsymbol \theta}^* R({\boldsymbol \theta}^{(t)})$ of the loss with regard to the model's parameters (_backward pass_) based on the training data in the batch.  -->
...
* _\textcolor{red}{Ridge/Lasso penalization}_: In neural networks this means adding an $L_2$ or $L_1$-penalty to the loss function to _penalize large weights_ \footnote{see chapter 10.7.2 in the course book}:
...
* _\textcolor{red}{SGD}_ is also a form of regularization (prevents over-fitting).
...
Two types of layers: 
   ...
   - _\textcolor{red}{Convolution layers}_ 
   - _\textcolor{red}{Pooling layers}_.
...
<!-- See for example here: https://clemsonciti.github.io/rcde_workshops/python_deep_learning/07-Convolution-Neural-Network.html -->
...
\scriptsize Figure 10.12 in @ISL
...
**To summarize: though double descent can sometimes occur in neural net- works, we typically do not want to rely on this behavior.** [James et al. Ch 10.8]
