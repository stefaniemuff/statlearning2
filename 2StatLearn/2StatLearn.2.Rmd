 ...
 * **Remember** for a univariate SV $X\sim f(x)$ the expected value (mean) is defined as
...
* For a **Random vector** $\boldsymbol{X}_{(p\times 1)}$,  the  mean vector $\boldsymbol{\mu}_{(p\times 1)}$ is 
...
**Note** that $\text{E}(X_j)$ is calculated from the marginal distribution of $X_j$ and contains no information about dependencies between $X_{j}$ and $X_k$, $k\neq j$.
...
$\boldsymbol{X}$ is a $2$-dimensional random vector with mean $\boldsymbol{\mu}$ and covariance matrix $\boldsymbol\Sigma$. 

Are these sufficient to totally describe the random vector  $\boldsymbol{X}$?
...
Why?^[Show the Datasaurus data: https://cran.r-project.org/web/packages/datasauRus/vignettes/Datasaurus.html]
