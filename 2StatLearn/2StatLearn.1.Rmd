...
![ISLR Figure 2.2](../../ISLR/Figures/Chapter2/2.1.pdf)
...
![ISLR Figure 2.2](../../ISLR/Figures/Chapter2/2.2.pdf)
...
\centering
![ISLR Figure 2.4](../../ISLR/Figures/Chapter2/2.4.pdf){width=43%}


![ISLR Figure 2.6](../../ISLR/Figures/Chapter2/2.6.pdf){width=45%}
...
We know that the true data generating model consist in a quadratic relationship between response $Y$ and covariate $x$
...
Try to fit a function to the observations _assuming we do not know_  the true relationship:

$~$

*  **poly1**: Simple linear model of the form $\beta_0+\beta_1 x$ fitted to the observations.  

<!-- _Underfits_ the data. -->

* **poly2**: Quadratic polynomial fit to the data, of the form $\beta_0+\beta_1 x +\beta_2 x^2$. 

<!-- This fits well. -->

* **poly10**: Polynomial of degree 10 fit of the form $\beta_0+\beta_1 x +\beta_2 x^2+\cdots +\beta_{10}x^{10}$

<!-- _Overfits_ the data. -->

* **poly20**: Polynomial of degree 10 fit of the form $\beta_0+\beta_1 x +\beta_2 x^2+\cdots +\beta_{20}x^{20}$ 
...
```{r ,echo=FALSE, fig.width=5.5, fig.height=4,fig.align = "center"}
data0 = data.frame( x0 = runif(50, min = -2, max = 4))
data0$y0 =  data0$x0^2 + rnorm(50, sd = 2)

p1 = ggplot() + geom_point(data = data, aes(x,y, color = "training"), size=0.7, alpha = 0.4) + 
  geom_point(data = data0, aes(x0,y0, color = "test"), size=1) + 
  geom_line(data = data, aes(x=x, y=predict(lm(y~x))), col="red")+ggtitle("poly1")+theme_minimal()

p2 = ggplot() + geom_point(data = data, aes(x,y, color = "training"), size=0.7, alpha = 0.4) +
  geom_point(data = data0, aes(x0,y0, color = "test"),size=0.7) + geom_line(data = data, aes(x=x, y=predict(lm(y~poly(x,2)))), col="orange")+ggtitle("poly2")+theme_minimal()

p3 =  ggplot() + geom_point(data = data, aes(x,y, color = "training"), size=0.7, alpha = 0.4) +
  geom_point(data = data0, aes(x0,y0, color = "test"),size=0.7) + geom_line(data = data, aes(x=x, y=predict(lm(y~poly(x, 10)))), col="pink")+ggtitle("poly10")+theme_minimal()

p4 =  ggplot() + geom_point(data = data, aes(x,y, color = "training"), size=0.7, alpha = 0.4) +
  geom_point(data = data0, aes(x0,y0, color = "test"),size=0.7) + geom_line(data = data, aes(x=x, y=predict(lm(y~poly(x, 20)))), col="purple")+ggtitle("poly20")+theme_minimal()
...
![ISLR Figure 2.12](../../ISLR/Figures/Chapter2/2.12.pdf)
