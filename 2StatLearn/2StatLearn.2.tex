...
\(\boldsymbol{X}\) is a \(2\)-dimensional random vector with mean
\(\boldsymbol{\mu}\) and covariance matrix \(\boldsymbol\Sigma\).

Are these sufficient to totally describe the random vector
\(\boldsymbol{X}\)?
...
Why?\footnote<.->{Show the Datasaurus data:
...
\protect\hypertarget{question-3-multivariate-normal-distribution}{}
\vspace{2mm}
...
\protect\hypertarget{question-4-independence}{}
\vspace{2mm}
