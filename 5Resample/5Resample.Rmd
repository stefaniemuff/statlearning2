...
\centering
![](../../ISLR/Figures/Chapter3/3.17.pdf){width=55%}
...
![ISLR Figure 5.1](../../ISLR/Figures/Chapter5/5.1.pdf){width=70%,centering}
...
```{r chunkName, echo=FALSE, fig.width=8, fig.height=4,fig.align = "center",out.width='70%'}
library(ISLR)
library(ggplot2)
library(ggpubr)
library(reshape2)
...
xx = seq(45,232)
fits = data.frame(horsepower = xx, matrix(nrow = length(xx), ncol = 11))
...
for (newsample in 1:11)
{
  ...
  for (polydeg in 1:10)
  {
    ...
    fits[,polydeg+1] = predict(lm.fit, data.frame(horsepower =  xx))
    testMSEmat[polydeg,newsample]=mean((Auto$mpg[-trainid]-lm.pred)^2)
  }
}
yrange=c(15,28)

fits %>% pivot_longer(-horsepower) %>%
  ggplot() + 
   geom_point(data = Auto %>% mutate(sel = sample(c(0,1),n, replace = T)), aes(horsepower, mpg, shape = factor(sel),
                               alpha = sel)) + 
  geom_line(aes(horsepower, value, group = name, color = name)) + scale_alpha(guide = 'none')
...
![ISLR Figure 5.3](../../ISLR/Figures/Chapter5/5.3.pdf){width=70%}

$k$-fold: 

![ISLR Figure 5.5](../../ISLR/Figures/Chapter5/5.5.pdf){width=70%}
...
\centering
![](../../ISLR/Figures/Chapter2/2.13.pdf){width=80%}
...
\centering
![](../../ISLR/Figures/Chapter5/5.7.pdf){width=50%}
...
\centering
![](../../ISLR/Figures/Chapter5/5.8.pdf){width=60%}
...
<!-- ![Training vs test error](pnas.pdf){width=105%, align=center} -->
...
dd = data.frame(Type = c("Treatment","Control"),
           Life_length = c( knitr::combine_words(treat, sep = ",", and = ""),  knitr::combine_words(control, sep = ",", and = "")),
           Mean = c(mean(treat),mean(control))
           )
knitr::kable(dd, digits = 2)
```


  * Is the difference between _means_ significant?
  
  * In the difference between _medians_ significant?
...
* For the means we can derive analytical solutions but not for the median

* If we would know the median  distribution $F$ , we could sample from $F$ ,
and use simulations to answer our question.

* However, without knowledge of the distribution, we cannot
calculate the standard deviation of our estimator
...
\normalsize
The median from **one bootstrap-sample**: 
\scriptsize
```{r, eval=TRUE, echo=TRUE}
boot1=sample(x=control,size=length(control),replace=TRUE)
median(boot1)
```
...
```{r, eval=TRUE, echo=TRUE}
B=1000
estimator_control=estimator_treat = rep(NA,B)
for (b in 1:B)
  {
  thisboot=sample(x=control,size=length(control),replace=TRUE)
  estimator_control[b]=median(thisboot)
  thisboot=sample(x=treat,size=length(treat),replace=TRUE)
  estimator_treat[b]=median(thisboot)
}
c(sd(estimator_control), sd(estimator_treat))
```
...
ggplot()+
  geom_histogram(data=data.frame(x=estimator_treat),aes(x=x, fill = "treatment"), alpha = 0.5) + 
  geom_histogram(data=data.frame(x=estimator_control),aes(x=x, fill = "control"), alpha = 0.5) 
...
* You find a template for the exercise sheets on our course website [https://wiki.math.ntnu.no/tma4268/2024v/subpage6](https://wiki.math.ntnu.no/tma4268/2024v/subpage6). Do not forget to install the packages listed at the beginning of the exercise sheet.
