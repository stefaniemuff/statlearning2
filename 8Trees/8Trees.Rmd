...
**But**: _\textcolor{red}{How to divide the predictor space}_ into non-overlapping regions $R_1,R_2,\ldots,R_J$? 
...
<!-- \vspace{2mm} -->
<!-- Below we see the partition of the `ozone` dataset given in the `ozone.tree`, where  -->
...
* If we have a dataset with many predictors or choose a stringent stopping criterion, we may fit a (too) large tree.  $\rightarrow$ _\textcolor{red}{overfitting}_? 
...
\scriptsize
```{r ozonecv, echo=TRUE, fig.width=6, fig.height=5,fig.align = "center",out.width='50%'}
set.seed(563)
...
<!-- In the [classification tree](classtree3) (grown with Gini index) there were 78 leaves - maybe we want a more easily interpretable tree? -->
...
Let's go from bagging to a random forest\footnote{n.b., why are we now speaking of a forest and no longer of a tree?}, using 4 ($\approx p/3$) randomly selected predictors for each tree:
