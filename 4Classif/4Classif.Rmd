...
\textcolor{orange}{Orange: default=yes}, \textcolor{blue}{blue: default=no}.
 ...
 \centering
![ISLR Figure 3.7](../../ISLR/Figures/Chapter4/4.1a.pdf){width=50%}
...
*  **Idea:** probability can be used for classification.

\vspace{0mm}

* Assume that we know the probability that a new observation $x_0$ belongs to class $k$, for $K$ classes $\mathcal{C} = \{c_1, c_2,\ldots, c_K\}$, with elements numbered as $1, 2, ..., K$
$$p_k(x_0) = \text{Pr}(Y=k | X=x_0), \quad k = 1, 2, ... K \ .$$
This is the probability that $Y=k$ given the observation $x_0$. 
\vspace{1mm}

* The _Bayes classifier_ assigns an observation to _\textcolor{red}{the most likely class}_, given its predictor values.  
...
* **Caveat:** We usually don't know the true conditional distribution $\text{Pr}(Y  |  X)$ for real data.
...
Remember: a _\textcolor{red}{good classifier}_ is a classifier that has a _\textcolor{red}{low test error}_ (why?).
...
![ISLR Figure 3.7](../../ISLR/Figures/Chapter4/4.2.pdf)
...
<!-- $\rightarrow$ If 'Student = TRUE' the odds to default decreases by a factor $\exp( \beta_{\text{studentYes}})=$ `r round(exp(summary(glm_default2)$coef[4,1]),2)` -->
---
...
For example: "What is the estimated probability that a student  with a balance of 2000, and an income of 40000 will default?"
...
* `SAheart` data set from the `bestglm` package, a retrospective sample of males in a heart-disease high-risk region in South Africa. 
...
* 160 cases (individuals who have suffered from a cororary heart disease) and 302 controls (individuals who have not suffered from a cororary heart disease).    
...
## Predictions for the first patients

```{r}
pred = predict(glm_heart , type="response")
head(cbind(d.heart, pred =  pred))
```
...
* We have discussed how to estimate $\text{Pr}(Y  |  X)$ with logistic regression (for two categories).
...
* Why? Because the nearest neighbors tend to be far away in high dimensions and the method is no longer  local. This is referred to as the _\textcolor{red}{curse of dimensionality}_.
...
We assume that $Y_i|X_i-x_i\sim\text{Ber}(p_i)$, where $p_i = P(Y_i =1|X_i = x_i)$ and that
...
# Two paradigms for classification models:


### 1. Diagnostic paradigm

The focus was  _directly_ estimating the posterior distribution for the classes $$\text{Pr}(Y=k \mid X=x)\ .$$ 
...
Here we use the Bayes theorem to _derive_ the posterior distribution for the classes 
$\text{Pr}(Y=k \mid X=x)$, using


  * A model  for  the _conditional distribution of predictors_ $f_k(x)=\text{Pr}(X=x \mid Y=k)$ for each class,

  * The _prior probabilities_ for each class $\pi_k=\text{Pr}(Y=k)$.
...
Given a continuous $X$ and categorical $Y$, we model: 
\vspace{1mm}
...
`round(pnorm(0,2,1.5))`=`r round(pnorm(0,2,1.5),2)`.
...
# How do we use this in observed data example?
...
# simulate test and training samples
...
\centering
![ISLR Figure 4.5](../../ISLR/Figures/Chapter4/4.5.pdf){width=70%}
...
```{r echo=FALSE, fig.width=6, fig.height=4.5,fig.align = "center",out.width='55%'}
n = 10000
x1 = rnorm(n, 1, sqrt(2))
x2 = rnorm(n, 1, sqrt(2))

gg = ggplot(data = data.frame(x = 1:n, y = cumsum(x2>4-x1)/(1:n))) + geom_line(aes(x,y)) +
  xlab("") + ylab("")
gg
```
...
tab = table(real=df$class, r.pred) 
tab
```
Estimated error `r (tab[1,2]+tab[2,1])/sum(tab)`
...
tab = table(real=df$class, predicted = r.pred) 
tab
```
...
\centering
![ISLR Figure 4.9](../../ISLR/Figures/Chapter4/4.9.pdf){width=100%}
...
unstable. 
    * if the number of observations $n$ is small and the distribution of the predictors $X$ is approximately (multivariate) normal.
...
* In practice the results are often
...
* [Videos on YouTube by the authors of ISL, Chapter 4,](https://www.youtube.com/playlist?list=PL5-da3qGB5IC4vaDba5ClatUmFppXLAhE)
